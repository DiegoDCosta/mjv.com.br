...
<div class="ts-vcsc-settings-group-header">
	<div class="display_header">
		<h2><span class="dashicons dashicons-image-filter"></span>Visual Composer Extensions - Element Usage Statistic</h2>
	</div>
	<div class="clear"></div>
</div>
	...
	<div class="ts-vcsc-icon-preview-wrap" style="margin-top: 0px;">
		<div class="ts-vcsc-section-main">
			...
			<div class="ts-vcsc-section-content">	
				...
				<div class="ts-vcsc-notice-field ts-vcsc-success" style="margin-top: 10px; font-size: 13px; text-align: justify;">
					Due to the modular framework of this plugin, you can enable and disable elements and features based on your needs. Naturally, the less elements and features you activate, the less resources this plugin
					requires, and the better its overall performance. The information shown below will give you a summary about which elements and features (based on their shortcodes), you are actually utiziling on your
					pages and posts. You can use that information and cross reference with the elements and features you have currently enabled, so you might be able to disable additional elements or features that you are
					not actually utilzing.
				</div>	
			</div>		
		</div>
		<div class="ts-vcsc-section-main">
			...
			<div class="ts-vcsc-section-content">	
				...
				<form id="ts-settings-statistics-compile-form" class="ts-settings-statistics-compile-form" data-lastscan="<?php echo $TS_VCSC_Shortcodes_LastCheck; ?>" name="oscimp_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" style="margin-bottom: 10px;">
					...
					<div class="ts-vcsc-notice-field ts-vcsc-warning" style="margin-top: 10px; font-size: 13px; text-align: justify;">
						Depending upon the total number of pages and posts on your site, the compilation of the usage statistic can take a short while. Please let the procedure finish and do not refresh this page while the
						compilation is still in process. For performance reason, the scan will only check for standalone and container elements, but not child elements, as child elements can not exist without a container element as well.
					</div>	
					<input type="submit" name="Statistics" id="ts-settings-statistics-compile-trigger" class="button button-primary" value="Compile New Usage Statistic" style="width: 225px;">
				</form>				
				<form id="ts-settings-statistics-autoset-form" class="ts-settings-statistics-autoset-form" data-lastscan="<?php echo $TS_VCSC_Shortcodes_LastCheck; ?>" name="oscimp_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" style="display: <?php echo ($TS_VCSC_Shortcodes_Validator == "true" ? "block" : "none"); ?>; margin-bottom: 10px;">
					...
					<div class="ts-vcsc-notice-field ts-vcsc-warning" style="margin-top: 10px; font-size: 13px; text-align: justify;">
						The last compilation scan revealed that you have currently some elements and/or post types activated, that are not actually used on any published page or post. Deactivating those elements and/or post types
						can improve overall site performance. Using the button below, the plugin will disable those elements and/or post types automatically, based on the results of the last compilation scan that occured on
					...
					<div class="ts-vcsc-notice-field ts-vcsc-success" style="margin-top: 10px; font-size: 13px; text-align: justify; display: <?php echo ($TS_VCSC_Shortcodes_ActivePostType > 0 ? "block" : "none"); ?>;">
						Some <span style="color: #7ad03a; font-weight: bold;">active</span> elements (<?php echo $TS_VCSC_Shortcodes_ActivePostType; ?>) are tied to custom post types, which often times provide multiple elements in order to use that post type within Visual Composer.
						If at least one of those post type associated elements is used, you will not be able to deactivate the other elements, as long as the post type itself remains active. Using the button below might therefore
						not be able to deactivate all currently unused but active elements, it those elements are tied to a custom post type that has at least one other of its elements utilized.
					</div>
					<input type="submit" name="Autosettings" id="ts-settings-statistics-autoset-trigger" class="button button-standard" value="(De)Activate All (Un)Used Elements" style="width: 225px; display: <?php echo (($TS_VCSC_Shortcodes_ActiveUnused - $TS_VCSC_Shortcodes_ActivePostType + $TS_VCSC_Shortcodes_InactiveUsed) > 0 ? "block" : "none"); ?>;">
				</form>
			</div>		
		</div>
		<div class="ts-vcsc-section-main" style="display: <?php echo ($TS_VCSC_Shortcodes_Validator == "true" ? "block" : "none"); ?>">
			...
			<div class="ts-vcsc-section-content clearFixMe">
				...
				?>
				<div class="ts-settings-statistics-table ts-datatables-container ts-datatables-style-yellow">
								...
								if ($TS_VCSC_Shortcodes_Validator == "true") {
									...
									foreach ($TS_VCSC_Shortcodes_UsePattern as $Shortcode => $statistics) {
										...
										echo '<tr data-units="' . $TS_VCSC_Shortcodes_UsePattern[$Shortcode]['total'] . '" data-total="' . $TS_VCSC_Shortcodes_TotalUsage . '">';
											...
											echo '<td>' . ($TS_VCSC_Shortcodes_Elements[$Shortcode]['introduced'] != "" ? $TS_VCSC_Shortcodes_Elements[$Shortcode]['introduced'] : "N/A") . '</td>';
											echo '<td>' . ($TS_VCSC_Shortcodes_Elements[$Shortcode]['retired'] != "" ? $TS_VCSC_Shortcodes_Elements[$Shortcode]['retired'] : "No") . '</td>';
